...
class EstimatorConfig:
    def __init__(self):
        self.simulation_batch_size = FLAGS.simulation_batch_size
        self.use_tpu = FLAGS.use_tpu
        self.tpu_name = FLAGS.tpu_name
        self.gcp_zone = FLAGS.gcp_zone
        self.gcp_project = FLAGS.gcp_project
        self.model_dir = FLAGS.model_dir
        if self.tpu_name and len(self.tpu_name.split("/")) > 1:
            self.tpu_name = self.tpu_name.split("/")[-1]
...
# TODO(njt): use in train*.py.
def get_estimator(estimator_config, model_fn, params):
    cluster = None
    if estimator_config.use_tpu and estimator_config.tpu_name:
        local_server_address = start_local_tf_server()
        cluster = tf.distribute.cluster_resolver.TPUClusterResolver(
            tpu=estimator_config.tpu_name,
            zone=estimator_config.gcp_zone,
            project=estimator_config.gcp_project,
    ...
    run_config = tf.compat.v1.estimator.tpu.RunConfig(
        cluster=cluster,
        model_dir=estimator_config.model_dir,
        tpu_config=tf.compat.v1.estimator.tpu.TPUConfig(
            per_host_input_for_training=is_per_host,
            num_cores_per_replica=1))

    estimator = tf.compat.v1.estimator.tpu.TPUEstimator(
        use_tpu=estimator_config.use_tpu,
        model_fn=model_fn,
        config=run_config,
        predict_batch_size=estimator_config.simulation_batch_size,
...
class Inference:
    ...
    def predict_slow(self, view_ids, action_ids):
        """Performs one-shot inference.

        very slow, used only in testing.
        """
        data_gen_builder = lambda sz: lambda: [{
                "unique_id": np.repeat(123456, sz),
                "view_ids": self._view_ids_padded_array([view_ids], sz),
                "action_ids": self._action_ids_padded_array([action_ids], sz)
        }]
        input_fn = self._input_fn_builder(data_gen_builder)
        inference_gen = self._estimator.predict(input_fn, yield_single_examples=False)
        pred = next(inference_gen)
        return pred
    ...
    def _view_ids_padded_array(self, view_ids, batch_size):
        return np.array(_nested_pad(view_ids,
            shape=[batch_size, self.config.max_seq_length],
            fill=0))
