...
def restructure(modality_sizes: ModalitySizeT,
                inputs: jnp.ndarray,
                key_order: Optional[Sequence[str]] = None) -> Mapping[str, jnp.ndarray]:
  ...
  """
  key_order = key_order or sorted(modality_sizes.keys())
  outputs = {}
  index = 0
  # Apply a predictable ordering to the modalities
  for modality in key_order:
...
class MultimodalPreprocessor(hk.Module):
  ...
  def __init__(
    ...
    self._key_order = key_order or sorted(self._modalities.keys())
  ...
  def __call__(self, inputs: jnp.ndarray, *,
    ...
    # Apply a predictable ordering to the modalities
    padded_ls = [padded[k] for k in self._key_order]
